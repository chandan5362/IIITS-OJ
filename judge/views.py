...
from .models import Problem,Solution,Result,Problem_Feature,Problem_Tags,Graph,Event,EventProblem
...
def judge(request):
    ...
    isActive = []
    for event in events:
        isActive.append(event.start_time.replace(tzinfo=None) <= datetime.datetime.now().replace(microsecond=0) <= event.end_time.replace(tzinfo=None))
    return render(request,'judge/judge.html',{'problems':problems,'events':list(zip(events,isActive))})

def event(request,event_id):
    e = Event.objects.get(id = event_id) 
    problems = e.problems.all()
    return render(request,'judge/eventproblem.html',{'problems':problems,'event':e})

def eventproblem(request,p_id):
    problem = EventProblem.objects.get(id = p_id)
    return render(request,'judge/eventproblemdetail.html',{'problem':problem})

def eventproblemsubmit(request,p_id):
    p=EventProblem.objects.get(id=p_id)
    context={
        'id':p.id,
        'title':p.title,
...
def accuracy_submission(pid,verdict):
    ...
    if verdict!="ce" and verdict!="cdt":
        if verdict=="ac" :
            ca=((f.accuracy*f.total_submission)/100)+1
            f.accuracy=(ca/(f.total_submission+1))*100
            before_dec, after_dec = str(f.accuracy).split('.')
            f.accuracy=float('.'.join((before_dec, after_dec[0:2])))
